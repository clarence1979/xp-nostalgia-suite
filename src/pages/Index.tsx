...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (session) {
        ...
        const freshKeys = await apiKeyStorage.fetchFreshApiKeys();
        apiCache.saveAll({
          OPENAI_API_KEY: freshKeys.OPENAI_API_KEY || '',
          CLAUDE_API_KEY: freshKeys.CLAUDE_API_KEY || '',
          GEMINI_API_KEY: freshKeys.GEMINI_API_KEY || '',
          REPLICATE_API_KEY: freshKeys.REPLICATE_API_KEY || '',
        });
      }
    }, 3000);
  ...
  return (
    ...
    >
      ...
      {/* Weather Widget */}
      {isAuthenticated && <WeatherWidget theme={theme} isAdmin={isAdmin} />}
      {isAuthenticated && <NewsWidget theme={theme} isAdmin={isAdmin} />}
      {isAuthenticated && <NoticesWidget theme={theme} isAdmin={isAdmin} />}
